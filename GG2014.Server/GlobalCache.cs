...
{   
    internal class GlobalCache 
    {
        ...
        private ObjectManager<string, User> userCache = new ObjectManager<string, User>(); // key:用户ID 。 Value：用户信息
        private ObjectManager<string, Group> groupCache = new ObjectManager<string, Group>();  // key:组ID 。 Value：Group信息
        ...
        public GlobalCache()
        {
            this.dbPersister = new VirtualDB();

            foreach (User user in this.dbPersister.GetAllUser())
            {
                this.userCache.Add(user.UserID, user);
            }

            foreach (Group group in this.dbPersister.GetAllGroup())
            {
        ...
        {
            User old = this.userCache.Get(user.UserID);
            if (old == null)
            {
                return;
            }
        ...
        {
            User user1 = this.userCache.Get(ownerID);
            User user2 = this.userCache.Get(friendID);
        ...
        {
            User user = this.userCache.Get(ownerID);
            if (user == null)
            {
                return ;
            }
        ...
        public List<Group> GetMyGroups(string userID)
        {
            ...
            {
                Group g = this.groupCache.Get(groupID);
                if (g != null)
                {
                    groups.Add(g);
                }
        ...
        public CreateGroupResult CreateGroup(string creatorID, string groupID, string groupName, string announce)
        {
            ...
            Group group = new Group(groupID, groupName, creatorID, announce, creatorID);            
            this.groupCache.Add(groupID, group);
            this.dbPersister.InsertGroup(group);

            User user = this.userCache.Get(creatorID);          
