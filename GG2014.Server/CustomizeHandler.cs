    ...
    internal class CustomizeHandler : IIntegratedCustomizeHandler 
    {
        ...
        void rapidServerEngine_MessageReceived(string sourceUserID, int informationType, byte[] info, string tag)
        {
            ...
            if (informationType == InformationTypes.UpdateUserInfo)
            {
                ...
                User old = this.globalCache.GetUser(user.UserID);               
                this.globalCache.UpdateUser(user);
                List<string> friendIDs = this.globalCache.GetFriends(sourceUserID);
                byte[] subData = ESPlus.Serialization.CompactPropertySerializer.Default.Serialize<User>(user.PartialCopy); //0922   
        ...
        {
            User user = this.globalCache.GetUser(data.UserID);
            if (user != null)
            {
        ...
        public byte[] HandleQuery(string sourceUserID, int informationType, byte[] info)
        {
            ...
            if (informationType == InformationTypes.AddFriend)
            {
                ...
                //0922
                User owner = this.globalCache.GetUser(sourceUserID);
                byte[] ownerBuff = CompactPropertySerializer.Default.Serialize<User>(owner);
            ...
            {
                List<string> contacts = this.globalCache.GetAllContacts(sourceUserID);
                Dictionary<string, User> contactDic = new Dictionary<string, User>();
                foreach (string friendID in contacts)
                {
                    if (!contactDic.ContainsKey(friendID))
                    {
                        User friend = this.globalCache.GetUser(friendID);
                        if (friend != null)
                        {
                ...
                return CompactPropertySerializer.Default.Serialize<List<User>>(new List<User>(contactDic.Values));
            }
            ...
            if (informationType == InformationTypes.GetSomeUsers)
            {
                ...
                return CompactPropertySerializer.Default.Serialize<List<User>>(friends);
            }
            ...
            if (informationType == InformationTypes.GetContactsRTData)
            {
                ...
                foreach (string friendID in contacts)
                {
                    ...
                    {
                        User data = this.globalCache.GetUser(friendID);
                        if (data != null)
                        {
            ...
            if (informationType == InformationTypes.GetUserInfo)
            {
                ...
                User user = this.globalCache.GetUser(target);
                if (user == null)
                {
                    return null;
                }
                ...
                return CompactPropertySerializer.Default.Serialize<User>(user);
            }     

            if (informationType == InformationTypes.GetMyGroups)
            {
                List<Group> myGroups = this.globalCache.GetMyGroups(sourceUserID);
